...
LINUX_DO_DISPLAY_NAME="${LINUX_DO_DISPLAY_NAME:-Linux Do}"
LINUX_DO_CLIENT_ID="${LINUX_DO_CLIENT_ID:-}"
LINUX_DO_CLIENT_SECRET="${LINUX_DO_CLIENT_SECRET:-}"
LINUX_DO_AUTHORIZATION_ENDPOINT="${LINUX_DO_AUTHORIZATION_ENDPOINT:-https://connect.linux.do/oauth2/authorize}"
LINUX_DO_TOKEN_ENDPOINT="${LINUX_DO_TOKEN_ENDPOINT:-https://connect.linux.do/oauth2/token}"
LINUX_DO_USER_ENDPOINT="${LINUX_DO_USER_ENDPOINT:-https://connect.linux.do/api/user}"
LINUX_DO_REDIRECT_URI="${LINUX_DO_REDIRECT_URI:-${API_URL%/}/api/callback}"
LINUX_DO_SCOPE="${LINUX_DO_SCOPE:-openid profile email}"
SMTP_HOST="${SMTP_HOST:-}"
...
ALL_PROXY="${ALL_PROXY:-}"
NO_PROXY="${NO_PROXY:-localhost,127.0.0.1,::1,api,web,pdf-renderer,db}"
http_proxy="${http_proxy:-}"
https_proxy="${https_proxy:-}"
all_proxy="${all_proxy:-}"
no_proxy="${no_proxy:-}"
...
write_env() {
  ...
  prepare_runtime_proxy_env
  {
    ...
    env_line CASDOOR_DISPLAY_NAME "${CASDOOR_DISPLAY_NAME}"
    ...
    env_line LINUX_DO_DISPLAY_NAME "${LINUX_DO_DISPLAY_NAME}"
    env_line LINUX_DO_CLIENT_ID "${LINUX_DO_CLIENT_ID}"
    env_line LINUX_DO_CLIENT_SECRET "${LINUX_DO_CLIENT_SECRET}"
    env_line LINUX_DO_AUTHORIZATION_ENDPOINT "${LINUX_DO_AUTHORIZATION_ENDPOINT}"
    env_line LINUX_DO_TOKEN_ENDPOINT "${LINUX_DO_TOKEN_ENDPOINT}"
    env_line LINUX_DO_USER_ENDPOINT "${LINUX_DO_USER_ENDPOINT}"
    env_line LINUX_DO_REDIRECT_URI "${LINUX_DO_REDIRECT_URI}"
    env_line LINUX_DO_SCOPE "${LINUX_DO_SCOPE}"
    ...
    env_line RUNTIME_PROXY "${RUNTIME_PROXY}"
    env_line HTTP_PROXY "${HTTP_PROXY}"
    env_line HTTPS_PROXY "${HTTPS_PROXY}"
    env_line ALL_PROXY "${ALL_PROXY}"
    env_line NO_PROXY "${NO_PROXY}"
    env_line http_proxy "${http_proxy}"
    env_line https_proxy "${https_proxy}"
    env_line all_proxy "${all_proxy}"
    env_line no_proxy "${no_proxy}"
...
prepare_runtime_proxy_env() {
  local proxy
  proxy="$(normalize_proxy_url "${RUNTIME_PROXY}")"
  if [[ -n "${proxy}" ]]; then
    HTTP_PROXY="${HTTP_PROXY:-${proxy}}"
    HTTPS_PROXY="${HTTPS_PROXY:-${proxy}}"
    ALL_PROXY="${ALL_PROXY:-${proxy}}"
  else
    HTTP_PROXY="$(normalize_proxy_url "${HTTP_PROXY}")"
    HTTPS_PROXY="$(normalize_proxy_url "${HTTPS_PROXY}")"
    ALL_PROXY="$(normalize_proxy_url "${ALL_PROXY}")"
  fi
